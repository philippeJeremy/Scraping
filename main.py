...
from comparaison import article_comparaison
from concurrent.futures import ThreadPoolExecutor
from PySide6.QtWidgets import (
    QApplication, QMainWindow, QVBoxLayout, QWidget, QLabel,
    QComboBox, QPushButton, QHBoxLayout, QCheckBox, QGroupBox
)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.site_functions = [site1, site2, site3, article_comparaison]
        ...
        label_instruction = QLabel(
            'Choisissez une fréquence pour exécuter le programme :')
    ...
    def schedule_program(self):
        ...
        if interval_ms > 0:
            ...
            # Appel immédiat de la fonction pour exécuter la tâche dès maintenant
            self.launch_program()

            # Planification du timer
            # Créez un QTimer si ce n'est pas déjà fait
            self.timer = QTimer(self)
            # Définir l'intervalle de temps
            self.timer.setInterval(interval_ms)
            # Connectez l'événement à la méthode d'exécution
            self.timer.timeout.connect(self.launch_program)
            self.timer.start()  # Démarrez le timer

            print(
                f"Programme exécuté immédiatement et planifié toutes les {frequency.lower()}.")
        else:
